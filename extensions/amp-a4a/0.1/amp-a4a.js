...
export class AmpA4A extends AMP.BaseElement {
    ...
    return utf8FromArrayBuffer(bytes).then(creative => {
      ...
      if (!creativeMetaData || !this.supportsShadowDom()) {
        ...
        try {
          ...
          shadowRoot.innerHTML += (cssBlock + bodyBlock);
          this.rendered_ = true;
   ...
   * @return {?creativeMetaData} Object result of parsing JSON data blob inside
   ...
   * @return {!creativeMetaData} if valid, null otherwise
   * @private
   */
   ...
   * @param {string} creative from which CSS is extracted
   * @param {!creativeMetaData} metaData Metadata object extracted from the
   ...
   * @param {string} creative from which CSS is extracted
   * @param {!creativeMetaData} meta data from creative.
   ...
   * @param {!creativeMetaData} metaData Reserialization metadata object.
   * @private
   */
  ...
  addCreativeExtensions_(metaData) {
    ...
    metaData.forEach(extension => {
      insertAmpExtensionScript(this.getWin(), extension, true);
    });
  }
}
