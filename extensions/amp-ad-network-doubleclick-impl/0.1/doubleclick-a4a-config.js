...
/**
 * Check whether Doubleclick supports the A4A rendering pathway for a given ad
 ...
 * @returns {boolean}  Whether Doubleclick should attempt to render via the A4A
 *   pathway.
 */
export function doubleclickIsA4AEnabled(win, element) {
  ...
  if (isDoubleclickA4AValidEnvironment(win)) {
  ...
  if (a4aParam && a4aParam.length > 1) {
    switch (a4aParam[1]) {
      ...
      case '2':
        ...
        break;
      default:
        dev.warn('doubleclick-a4a-config', 'Unknown a4a URL parameter: ',
            params['a4a']);
    }
  }
}
...
/**
 * Check whether Doubleclick supports the A4A rendering pathway is valid for the
 ...
 *   pathway.
 */
export function isDoubleclickA4AValidEnvironment(win) {
