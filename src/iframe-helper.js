 ...
 * @return {!UnlistenDef}
 */
export function listenFor(iframe, sentinel, typeOfMessage, callback, opt_is3P,
 ...
 */
export function listenForOncePromise(iframe, sentinel, typeOfMessages,
                                     opt_is3P) {
  const unlistenList = [];
  ...
  return new Promise(resolve => {
    for (let i = 0; i < typeOfMessages.length; i++) {
      ...
      const unlisten = listenFor(iframe, sentinel, message,
                                 (data, source, origin) => {
                                   for (let i = 0; i < unlistenList.length;
                                        i++) {
                                     unlistenList[i]();
                                   }
 ...
 * @param {boolean=} opt_is3P set to true if the iframe is 3p.
 */
export function postMessage(iframe, sentinel, type, object, targetOrigin,
                            opt_is3P) {
 ...
 * @param {boolean=} opt_is3P set to true if the iframe is 3p.
 */
export function postMessageToWindows(iframe, sentinel, targets, type,
                                     object, opt_is3P) {
...
export class SubscriptionApi {
  ...
  constructor(iframe, sentinel, type, is3p, requestCallback) {
    ...
    this.unlisten_ = listenFor(this.iframe_, sentinel, type,
                               (data, source, origin) => {
                                 // This message might be from any window within
                                 // the iframe, we need to keep track of which
                                 // windows want to be sent updates.
                                 if (!this.clientWindows_.some(entry =>
                                     entry.win == source)) {
                               ...
                               // For 3P frames we also allow nested
                               // frames within them to subscribe..
